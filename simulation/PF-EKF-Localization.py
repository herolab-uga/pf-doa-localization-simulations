...
R = np.array([[0.10, 0], [0, 0.10]])
ekf = ExtendedKalmanFilter(F = F, H = H, Q = Q, R = R, P = P)
predictions = []
...
for i in range(0,len(original_tragectory)):
    ...
    pred = np.dot(H,  ekf.predict())[0]
    ekf.update(pos)
    # print("pred: ",pred,"\t pos: ",pos)
    previous_errors.append(errors[min_index])
    if i > 0:
        distance_error.append(dist(pred[0],pred[1],original_tragectory[i]))
